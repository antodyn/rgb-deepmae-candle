...
impl DeepMAEL {
    ...
    fn random_masking(
        ...
        let dev = xs.device();
        let noise =
            Tensor::rand(0.0f32, 1.0f32, candle_core::Shape::from_dims(&[n, l]), dev).unwrap();
        // rand noise
        let (ids_shuffle, ids_restore) = tensor_ops::tensor_argsort(&noise, dev).unwrap();
        ...
        // mask
        let mask_0 = Tensor::zeros((n, len_keep), DType::F32, dev).unwrap();
        let mask_1 = Tensor::ones((n, l - len_keep), DType::F32, dev).unwrap();
    ...
    fn forward_decoder(&self, xs: &Tensor, ids_restore: &Tensor) -> Result<Tensor> {
        ...
        let mut xs = xs.broadcast_add(&self.decoder_pos_embed)?; // add pos embed
